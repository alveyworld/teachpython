...
class GameBoard(game_mouse.Game):
    ...
    def paint(self, surface):
        if self.end:
            ...
            if self.turn == 's':
                pass#self.turn = 'x'
            elif self.pos != None and self.board[int(self.pos)] == 0:
        ...
        surface.blit(self.label, (int(self.width/2)-150, int(self.height/2)-50))
        return
...
def processClick(board, click_x, click_y, turn, width, height):
    pos = int(click_y/int(height/3))*3 + int(click_x/int(width/3))
    spot = pygame.Rect((pos%3)*int(width/3)+int(width/6)-50,
                       int(pos/3)*int(height/3)+int(height/6)-50, 100, 100)

    return spot, pos

def drawXO(surface, width, height, turn, square, board):
    spot = pygame.Rect((square%3)*int(width/3)+int(width/6)-50,
                       int(square/3)*int(height/3)+int(height/6)-50, 100, 100)
    center_point = [(square%3)*int(width/3)+int(width/6),
                    int(square/3)*int(height/3)+int(height/6)]
    cp = center_point
    if turn == "x":
...
def drawWindow(surface, width, height, board):
    ...
    for x in range(3):
        for y in range(3):
            outline = pygame.Rect(x*int(width/3), y*int(height/3), int(width/3), int(height/3))
